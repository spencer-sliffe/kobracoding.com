...
class FSPostManager {
    ...
    updateLikeCount(post, likeCount, userId, isAdding) {
        const postId = post.id;
        const query = this.db.collection(this.postsCollection).where("id", "==", postId);
    ...
    deletePost(post, completion) {
        ...
        query.get().then((querySnapshot) => {
            if (querySnapshot.empty) {
                completion(new Error("No document found with matching id"), null);
            } else {
                const document = querySnapshot.docs[0];
    ...
    updateComments(post, comment) {
        ...
        query.get().then((querySnapshot) => {
            ...
            document.ref.collection("comments").add(commentData).then(() => {
